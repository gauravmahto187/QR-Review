...
export default async function QuestionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl">
    ...
    <div className="mt-3"><p className="text-sm font-semibold text-emerald-700">Review questions</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Question flow</h1><p className="mt-2 text-sm leading-6 text-slate-600">{active} active of {data.questions.length} total · Maximum 5 active questions</p></div>
    <section className="mt-7"><QuestionManager businessId={data.business.id} questions={data.questions} /></section>
    <section className="mt-9"><div className="mb-4 flex items-center gap-2"><Eye className="size-5 text-slate-500" /><div><h2 className="text-lg font-semibold text-slate-950">Mobile flow preview</h2><p className="text-xs text-slate-500">Admin-only structural preview</p></div></div><QuestionPreview questions={data.questions} /></section>
  </div>;
}
