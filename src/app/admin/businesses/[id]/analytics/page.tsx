...
export default async function BusinessAnalyticsPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="mx-auto max-w-3xl pb-8">
    ...
    {!hasActivity ? <section className="mt-6 rounded-[1.75rem] border border-slate-200 bg-white p-7 text-center shadow-sm"><BarChart3 className="mx-auto size-9 text-slate-300" /><h2 className="mt-4 text-lg font-semibold text-slate-950">No activity yet</h2><p className="mt-2 text-sm leading-6 text-slate-500">Share or scan this business QR to start collecting analytics.</p><Link className="mt-5 inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-6 text-sm font-semibold text-white" href={`/admin/businesses/${business.id}/qr`}><QrCode className="size-5" />View QR</Link></section> : <>
      ...
      <section className="mt-8 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><h2 className="text-lg font-semibold text-slate-950">Daily trend</h2><p className="mt-1 text-xs text-slate-500">Asia/Kathmandu calendar dates · {range.label}</p><TrendChart points={analytics.trend} /></section>
