...
import { ArrowLeft, ArrowRight, Check, Languages, LoaderCircle, Pencil, RefreshCw, Sparkles } from "lucide-react";
...
async function requestReview(body: Record<string, string>): Promise<PublicReviewActionState> {
  ...
  return response.json() as Promise<PublicReviewActionState>;
}

export function PublicReviewFlow({ initialGeneration, initialSession, questions, slug }: { initialGeneration: PublicGeneration | null; initialSession: PublicSession | null; questions: PublicQuestion[]; slug: string }) {
  const [session, setSession] = useState(initialSession);
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  if (!session) return <div className="mt-7"><p className="text-center text-sm leading-6 text-slate-600">Answer a few quick questions to prepare your review.</p>{error ? <ErrorMessage message={error} /> : null}<button className="mt-6 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-5 text-base font-semibold text-white shadow-lg shadow-emerald-900/10 transition hover:bg-emerald-800 disabled:opacity-70" disabled={pending} onClick={() => startTransition(async () => { const result = await requestReview({ action: "START", slug }); if (result.session) setSession(result.session); else setError(result.error ?? "We couldn’t start your review."); })} type="button"><Sparkles className="size-5" />{pending ? "Starting…" : "Start review"}</button><p className="mt-4 text-center text-xs leading-5 text-slate-400">No login or personal information required.</p></div>;
...
function ActiveReviewFlow({ initialGeneration, initialSession, questions, slug }: { initialGeneration: PublicGeneration | null; initialSession: PublicSession; questions: PublicQuestion[]; slug: string }) {
  ...
  if (generation) return <GeneratedReviewResult error={error} generation={generation} key={generation.generationNumber} onRegenerate={runGeneration} pending={pending} />;
  ...
  const selected = question ? session.answers[question.id] : null;
  ...
  function chooseAnswer(questionId: string, optionId: string) {
    ...
    startTransition(async () => {
      ...
      else setError(result.error ?? "That answer could not be saved.");
    });
  }
...
function GeneratedReviewResult({ error, generation, onRegenerate, pending }: { error: string | null; generation: PublicGeneration; onRegenerate: () => void; pending: boolean }) {
  ...
  return <div className="mt-7"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700"><Check className="size-5" /></span><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-700">Your review is ready</p><h2 className="mt-1 text-xl font-semibold text-slate-950">Make it sound like you</h2></div></div><p className="mt-4 flex items-center gap-2 text-xs font-medium text-slate-500"><Languages className="size-4" />{generation.language === "ne" ? "Nepali" : "English"} · Version {generation.generationNumber} of 2</p>
    <label className="mt-5 block text-sm font-semibold text-slate-700" htmlFor="generated-review"><span className="inline-flex items-center gap-2"><Pencil className="size-4" />Edit your review</span></label><textarea className="mt-2 min-h-52 w-full resize-y rounded-2xl border border-slate-200 bg-white p-4 text-base leading-7 text-slate-800 shadow-inner outline-none transition focus:border-emerald-600 focus:ring-4 focus:ring-emerald-600/10" id="generated-review" maxLength={1200} onChange={(event) => setText(event.target.value)} value={text} /><p className="mt-2 text-xs leading-5 text-slate-400">You can edit this freely without using another AI generation.</p>{error ? <ErrorMessage message={error} /> : null}
    {generation.canRegenerate ? <button className="mt-5 flex min-h-13 w-full items-center justify-center gap-2 rounded-2xl border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-700" onClick={onRegenerate} type="button"><RefreshCw className="size-4" />Regenerate another version</button> : <p className="mt-5 rounded-2xl bg-slate-50 p-4 text-center text-sm text-slate-500">You’ve used the available regeneration. You can still edit this version.</p>}<button className="mt-3 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-5 text-base font-semibold text-white" onClick={() => setHandoffMessage(true)} type="button">Continue<ArrowRight className="size-5" /></button>{handoffMessage ? <p className="mt-3 rounded-2xl bg-sky-50 p-4 text-center text-sm font-medium text-sky-800" role="status">Google handoff will be available next.</p> : null}
  </div>;
}
