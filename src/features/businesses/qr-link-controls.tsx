...
export function QrLinkControls({ businessId, publicUrl, slug, purpose = "review" }: { businessId: string; publicUrl: string; slug: string; purpose?: "review" | "smart" }) {
  ...
  return <div>
    ...
    <div className="mt-5 grid grid-cols-2 gap-3"><a className="flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-emerald-700 px-3 text-sm font-semibold text-white" download={filename(slug, "png")} href={`/api/admin/businesses/${businessId}/${route}?format=png`}><Download className="size-4" />PNG</a><a className="flex min-h-13 items-center justify-center gap-2 rounded-2xl bg-slate-900 px-3 text-sm font-semibold text-white" download={filename(slug, "svg")} href={`/api/admin/businesses/${businessId}/${route}?format=svg`}><Download className="size-4" />SVG</a></div>
  </div>;
}
