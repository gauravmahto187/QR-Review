...
export function DeleteBusinessButton({ businessId, businessName, cleanup = false }: { businessId: string; businessName: string; cleanup?: boolean }) {
  ...
  const [confirmation, setConfirmation] = useState(cleanup ? businessName : "");
  ...
  return <>
    ...
    <dialog ref={dialog} onCancel={event => { if (pending) event.preventDefault(); }} className="m-auto w-[calc(100%-1.5rem)] max-w-md rounded-3xl bg-white p-6 shadow-2xl backdrop:bg-slate-950/50" aria-label="Delete business confirmation">
      ...
      <p className="mt-3 break-words text-sm leading-6 text-slate-600">{cleanup || state.deleted ? `${businessName} has been deleted. Retry removing its remaining uploaded files.` : `This permanently deletes ${businessName}, its subscriptions, reviews, questions, Smart Links, payment methods, analytics, and uploaded images. Its QR links will stop working. This cannot be undone.`}</p>
      <form action={action} onReset={event => event.preventDefault()} className="mt-5 space-y-4" aria-busy={pending}>
        <input type="hidden" name="businessId" value={businessId} />
        {cleanup || state.deleted ? <input type="hidden" name="confirmation" value={businessName} /> : <label className="block text-sm font-semibold text-slate-700">Type the business name to confirm<input autoComplete="off" name="confirmation" value={confirmation} onChange={event => setConfirmation(event.target.value)} disabled={pending} required className="mt-2 min-h-12 w-full rounded-xl border border-slate-300 px-3 text-base" /></label>}
        {state.error && <p role="alert" className="text-sm text-rose-700">{state.error}</p>}
        <div className="grid grid-cols-2 gap-3"><button type="button" disabled={pending} onClick={() => dialog.current?.close()} className="min-h-12 rounded-xl border border-slate-300 px-3 text-sm font-semibold disabled:opacity-50">Cancel</button><button type="submit" disabled={pending || (!(cleanup || state.deleted) && confirmation !== businessName)} className="min-h-12 rounded-xl bg-rose-700 px-3 text-sm font-semibold text-white disabled:opacity-50">{pending ? <PendingLabel>{cleanup || state.deleted ? "Cleaning up…" : "Deleting…"}</PendingLabel> : cleanup || state.deleted ? "Retry cleanup" : "Delete Business"}</button></div>
      </form>
    </dialog>
  </>;
}
